...
class PreferencesNotifythisDialog(gtk.Dialog):
    __gtype_name__ = "PreferencesNotifythisDialog"
    prefernces = {}

    def __init__(self):
        """__init__ - This function is typically not called directly.
        Creation of a PreferencesNotifythisDialog requires redeading the associated ui
        file and parsing the ui definition extrenally,
        and then calling PreferencesNotifythisDialog.finish_initializing().

        Use the convenience function NewPreferencesNotifythisDialog to create
    ...
    def get_preferences(self):
        ...
        preferences for notifythis. Creates a couchdb record if
        necessary.
        """
...
def NewPreferencesNotifythisDialog():
    """NewPreferencesNotifythisDialog - returns a fully instantiated
    PreferencesNotifythisDialog object. Use this function rather than
    creating a PreferencesNotifythisDialog instance directly.
    """

    #look for the ui file that describes the ui
    ui_filename = os.path.join(getdatapath(), 'ui', 'PreferencesNotifythisDialog.ui')
    if not os.path.exists(ui_filename):
        ui_filename = None

    builder = gtk.Builder()
    builder.add_from_file(ui_filename)
    dialog = builder.get_object("preferences_notifythis_dialog")
    dialog.finish_initializing(builder)
    return dialog

if __name__ == "__main__":
    dialog = NewPreferencesNotifythisDialog()
    dialog.show()
    gtk.main()
